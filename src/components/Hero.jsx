    ...
    <section className="relative overflow-hidden glow-emerald glow-emerald-soft">
      ...
      <div className="relative max-w-7xl mx-auto px-5 lg:px-8 pt-14 lg:pt-24 pb-16 lg:pb-24 grid lg:grid-cols-12 gap-10 items-center">
        ...
        <div className="lg:col-span-7">
          ...
          <h1 className="mt-5 text-[40px] md:text-[58px] leading-[1.04] font-extrabold tracking-tight text-zinc-50">
            Healthcare that <br className="hidden md:block" />
            <span className="font-display italic font-medium text-emerald-400">
              comes to you.
            </span>
          </h1>
          ...
          <div className="mt-7 flex flex-wrap items-center gap-3">
            ...
            <a
              ...
              className="inline-flex items-center gap-2 h-12 px-5 rounded-full border border-white/15 bg-white/[0.03] text-zinc-100 hover:bg-white/[0.07] font-semibold text-[14px] transition-colors"
            >
...
function Stat({ label, value }) {
  return (
    <div className="glass rounded-2xl p-3.5">
      <p className="text-[20px] font-extrabold tracking-tight text-emerald-300">
        {value}
      </p>
      <p className="text-[11px] mt-0.5 uppercase tracking-[0.12em] text-zinc-400 font-semibold">
        {label}
      </p>
    </div>
  );
...
function FloatingCard({ className = '', delay = '0s', icon, title, desc }) {
  return (
    <div
      className={
        'absolute glass rounded-2xl p-4 max-w-[260px] animate-float ' + className
      }
      style={{ animationDelay: delay }}
    >
      <div className="flex items-center gap-2">
        <span className="h-8 w-8 rounded-lg bg-emerald-500/15 border border-emerald-500/30 flex items-center justify-center">
          {icon}
        </span>
        <p className="text-[14px] font-bold text-zinc-50">{title}</p>
      </div>
      <p className="mt-2 text-[12.5px] leading-relaxed text-zinc-300">{desc}</p>
    </div>
  );
}
